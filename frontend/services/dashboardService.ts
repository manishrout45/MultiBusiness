...
export const dashboardService = {
  ...
  async listAdminCategories(token?: string | null): Promise<AdminCategory[]> {
    ...
    return [
      { id: '1', name: 'Retail Store', slug: 'retail-store', description: 'General retail', isActive: 1 },
      { id: '2', name: 'Electronics', slug: 'electronics', description: 'Gadgets', isActive: 1 },
      { id: '3', name: 'Restaurant', slug: 'restaurant', description: 'Food & dining', isActive: 1 },
    ];
  },
  ...
  async createCategory(
    ...
    if (token) {
      const res = await apiRequest<{ data: { id: number; name: string; slug: string; description?: string } }>(
        '/admin/categories',
  ...
  async updateCategory(
    ...
    if (token) {
      await apiRequest(`/admin/categories/${id}`, { method: 'PATCH', token, body: input });
    }
  },
